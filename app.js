(function(){
  ...
  function openSide(){
    ...
    try { maybePromptGPSOnMenu(); } catch(_) {}
  }
  function closeSide(){
    if(!el.sideDrawer || !el.sideOverlay) return;
  ...
  async function tryGetPositionWithFallback(){
    // First try: high accuracy, short timeout
    try{
      return await getCurrentPositionAsync({ enableHighAccuracy: true, timeout: 10000, maximumAge: 0 });
    }catch(e1){
      // Second try: allow cached data and lower accuracy with longer timeout
      try{
        return await getCurrentPositionAsync({ enableHighAccuracy: false, timeout: 20000, maximumAge: 30000 });
      }catch(e2){
        throw e2 || e1;
      }
    }
  ...
  async function ensureWithinGeofenceOrExplain(){
    try{
      const pos = await tryGetPositionWithFallback();
      const { latitude, longitude } = pos.coords || {};
      if(typeof latitude !== 'number' || typeof longitude !== 'number'){
        toast('تعذر تحديد الموقع');
        return false;
      }
      const d = distanceMeters(latitude, longitude, GEOFENCE_CENTER.lat, GEOFENCE_CENTER.lon);
      if(d > GEOFENCE_RADIUS_M){
        toast('يتعذر إرسال الطلب');
        return false;
      }
      return true;
    }catch(e){
      // Give an accurate hint based on permission state if available
      try{
        if(navigator.permissions && navigator.permissions.query){
          const st = await navigator.permissions.query({ name: 'geolocation' });
          if(st.state === 'granted'){
            toast('تعذر تحديد الموقع حالياً');
          } else if(st.state === 'denied'){
            toast('يرجى منح إذن الموقع من إعدادات المتصفح');
          } else {
            toast('يرجى السماح بالوصول إلى الموقع');
          }
        } else {
          toast('تعذر تحديد الموقع');
        }
      }catch(_){ toast('تعذر تحديد الموقع'); }
      return false;
    }
  }

  async function maybePromptGPSOnMenu(){
    if(state.promptedGPS) return;
    state.promptedGPS = true;
    try{
      await tryGetPositionWithFallback();
      // If success, do nothing
    }catch(_){
      // Soft reminder without insisting that GPS is off
      toast('تعذر تحديد الموقع');
    }
  }
      ...
      if(window.FIREBASE_CONFIG && window.firebase && 'serviceWorker' in navigator && window.Notification){
        ...
        messaging.onMessage((payload) => {
          try{
            ...
            const tableNum = d.table || d.table_number || d.tableId || d.table_id;
            const title = tableNum ? `طلب للطاولة : ${tableNum}` : (payload?.notification?.title || 'إشعار');
            toast(title);
          }catch(_){ }
        });
  ...
  async function createOrder(customerName){
    ...
    const within = await ensureWithinGeofenceOrExplain();
    if(!within) return;
